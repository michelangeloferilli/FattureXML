...
class FatturaViewer(tk.Tk):
    ...
    def create_edit_fields(self):
        ...
        for section_title, fields in left_sections:
            ...
            for i, (xpath, label) in enumerate(fields):
                ...
                if xpath in date_fields:
                    ...
                    # Per i campi non-data, usa Entry normale
        ...
        for section_title, fields in right_sections:
            ...
            aliquota_iva_widget = None
            imponibile_widget = None
            imposta_widget = None
            ...
            for i, (xpath, label) in enumerate(fields):
                ...
                if "AliquotaIVA" in xpath:
                    ...
                    if section_title == "Dati Riepilogo":
                        aliquota_iva_widget = entry_widget
                ...
                # Verifica se è un campo data
                    ...
                    if section_title == "Dati Riepilogo":
                        if "ImponibileImporto" in xpath:
                            imponibile_widget = entry_widget
                        elif "Imposta" in xpath:
                            imposta_widget = entry_widget
            ...
            if section_title == "Dati Riepilogo" and aliquota_iva_widget and imponibile_widget and imposta_widget:
                # Salva i riferimenti ai widget come attributi della classe
                self.aliquota_iva_widget = aliquota_iva_widget
                self.imponibile_widget = imponibile_widget
                self.imposta_widget = imposta_widget
                
                # Crea una variabile per tracciare le modifiche
                self.aliquota_var = tk.StringVar()
                self.aliquota_var.set(aliquota_iva_widget.get())  # Valore iniziale
                aliquota_iva_widget.config(textvariable=self.aliquota_var)
                
                # Doppio binding: sia all'evento che alla variabile
                aliquota_iva_widget.bind("<<ComboboxSelected>>", self.calcola_imposta)
                self.aliquota_var.trace_add("write", lambda *args: self.calcola_imposta())
                
                # Evento per l'imponibile quando perde il focus
                imponibile_widget.bind("<FocusOut>", self.calcola_imposta)
                
                # Evento per l'imponibile quando viene premuto Invio
                imponibile_widget.bind("<Return>", self.calcola_imposta)
                
                # Colora di grigio chiaro il campo imposta per indicare che è calcolato automaticamente
                imposta_widget.config(bg="#f0f0f0")
